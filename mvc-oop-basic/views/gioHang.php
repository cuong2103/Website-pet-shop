                                        ...
                                        <tr>
                                            ...
                                            <td class="pro-price">
                                                <?php if($sanPham['gia_khuyen_mai']){ ?>
                                                    <span><?= formmatPrice($sanPham['gia_khuyen_mai']) ?></span>
                                                <?php } else{ ?>
                                                    <span><?= formmatPrice($sanPham['gia_san_pham']) ?></span>
                                                <?php } ?>
                            ...
                            <div class="cart-calculator-wrapper">
                                <div class="cart-calculate-items">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table">
                                            <tr>
                                                <td>Tông tiền sản phẩm</td>
                                                <td><?= formmatPrice($tongGioHang) ?></td>
                                            </tr>
                                            <tr>
                                                <td>Vận chuyển</td>
                                                <td>40.000 VNĐ</td>
                                            ...
                                            <tr class="total">
                                                ...
                                                <td class="total-amount"><?= formmatPrice($tongGioHang + 40000) ?></td>
                                            </tr>
                                        </table>
                                    </div>
                                </div>
...
$(document).on('change', '.pro-qty input', function() {
    let san_pham_id = $(this).data('id');
    let so_luong = $(this).val();

    $.post('?act=update-so-luong', { san_pham_id, so_luong }, function(response) {
        location.reload();
    }, 'json');
});
...
$(document).on('click', '.pro-remove a', function(e) {
    ...
    $.post('?act=remove-san-pham', { san_pham_id }, function(response) {
        location.reload();
    }, 'json');
});
</script>
